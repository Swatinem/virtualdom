...
function diffChildren(from, to, node, patches) {
	...
	if (!from.children || !from.children.length) {
		if (!to.children)
			return ret;
		return to.children.map(function (c) {
			return {type: 'insert', child: c};
		});
	}
	// only removals
	if (!to.children || !to.children.length) {
		return from.children.map(function () {
			return {type: 'remove', index: 0};
		});
	}

	// general operations
	var matrix = levenshtein.matrix(from.children, to.children, keyFunction);
	var ops = levenshtein.operations(matrix);
	ops.forEach(function (op) {
		if (op.type === 'noop')
			return diffElem(from.children[op.fromindex], to.children[op.index],
			                node.concat([op.fromindex]), patches);
		if (op.type !== 'remove')
			op.child = to.children[op.index];
		ret.push(op);
	});
	return ret;
}
