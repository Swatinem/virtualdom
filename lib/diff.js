...
function diffArray(from, to) {
	...
	from.filter(function (id) { return id; }).forEach(function (key) { fromSet[key] = true; });
	to.filter(function (id) { return id; }).forEach(function (key) { toSet[key] = true; });
...
function diffChildren(from, to, node, patches) {
	...
	while (head < from.length && head < to.length &&
	       keyFunction(from[head]) === keyFunction(to[head])) {
		diffElem(from[head], to[head], node.concat([head]), patches);
		head++;
	}
	var tail = 0;
	while (head + tail < from.length && head + tail < to.length &&
	       keyFunction(from[from.length - 1 - tail]) === keyFunction(to[to.length - 1 - tail])) {
		diffElem(from[from.length - 1 - tail], to[to.length - 1 - tail], node.concat([from.length - 1 - tail]), patches);
		tail++;
	}

	// general operations
	var matrix = levenshtein.matrix(from.slice(head, from.length - tail), to.slice(head, to.length - tail), keyFunction);
