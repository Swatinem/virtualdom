...
function applyPatch(node, patch) {
	...
	if (patch.addClasses)
		patch.addClasses.forEach(node.classList.add.bind(node.classList));
	if (patch.removeClasses)
		patch.removeClasses.forEach(node.classList.remove.bind(node.classList));
	...
	if (patch.childPatches)
		patch.childPatches.forEach(applyChildPatch.bind(this, node));
}

function applyChildPatch(node, patch) {
	var newNode = patch.node && toDOM(patch.node);
	switch (patch.type) {
	case 'insert':
		if ('index' in patch)
